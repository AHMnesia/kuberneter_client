param(
    [string]$Namespace = 'suma-ecommerce-client',
    [string]$DeploymentName = 'suma-ecommerce-client',
    [string]$ImageName = 'suma-ecommerce-client',
    [string]$Version = 'v1.0.0',
    [switch]$NoCache,
    [switch]$NonInteractive
)

Write-Host "=== Suma Ecommerce Client: Build & Redeploy ===" -ForegroundColor Green
...
function Get-Paths {
    ...
    $serviceRoot = Join-Path $workspaceRoot 'suma-ecommerce-client'
    $dockerfile = Join-Path $serviceRoot 'Dockerfile'
    $k8sSvcDir = Join-Path $k8sRoot 'suma-ecommerce-client'
...
function Apply-ClientStack {
    ...
    @('namespace.yaml','service.yaml','deployment.yaml','hpa.yaml') | ForEach-Object {
        $f = Join-Path $Dir $_
...
Apply-ClientStack -Ns $Namespace -Dir $paths.Manifests
