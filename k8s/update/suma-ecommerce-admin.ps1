param(
    [string]$Namespace = 'suma-ecommerce-admin',
    [string]$DeploymentName = 'suma-ecommerce-admin',
    [string]$ImageName = 'suma-ecommerce-admin',
    [string]$Version = 'v1.0.0',
    [switch]$NoCache,
    [switch]$NonInteractive
)

Write-Host "=== Suma Ecommerce Admin: Build & Redeploy ===" -ForegroundColor Green
...
function Get-Paths {
    ...
    $serviceRoot = Join-Path $workspaceRoot 'suma-ecommerce-admin'
    $dockerfile = Join-Path $serviceRoot 'Dockerfile'
    $k8sSvcDir = Join-Path $k8sRoot 'suma-ecommerce-admin'
...
Apply-AdminStack -Ns $Namespace -Dir $paths.Manifests
